 ...
 * 
 * @param {Array} espnData - An array of arrays where each inner array represents a week of games, and contains objects representing individual games.
 * @return {Array} - A new array of arrays where each inner array contains only game objects with 'Day of Week' property not equal to 'Thursday', and no empty arrays are present.
 */
function removeThursdayGames(espnData) {
  return espnData
    .map((arrayOfWeeklyGames) => {
      return arrayOfWeeklyGames.filter((item) => item['Day of Week'] !== "Thursday");
    })
    .filter((arrayOfWeeklyGames) => arrayOfWeeklyGames.length > 0);
}
