...
function writeTopChoicesToFile(topChoices, filePrefix = '') {
  ...
  const outputData = topChoices.map((item, index) => {
    const gameDetails = item.combination.map((game, gameIndex) => {
      return [
        game["Week"].toString().padStart(3, " "),
        game["Projected Winner"].toString().padEnd(25, " "),
        game["Projected Winner's Rank"].toString().padStart(3, " "),
        (parseFloat(game["Projected Winner's Percentage"]).toFixed(1) + '%').padStart(7, " "),
        (parseFloat(game["Projected Winner's Percentage Difference"]).toFixed(1) + '%').padStart(7, " "),
      ].join('|');
    }).join('\n');
